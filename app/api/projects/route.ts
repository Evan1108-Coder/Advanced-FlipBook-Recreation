...
export async function POST(request: Request) {
  ...
  const sourceInputs = Array.isArray(body.sources)
    ...
    : [];
  if (settings.sourceUrlRequirement === "required" && !sourceInputs.some((source) => source.url)) {
    return NextResponse.json({ error: "At least one source URL is required by settings." }, { status: 400 });
  }
